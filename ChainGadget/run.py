...
class Router(Switch):
    ...
    def start(self, controllers):
        ...
        self.waitOutput()
        sleep(0.1)
        self.cmd("/usr/lib/frr/zebra -f ./configs/%s.conf -d -i /tmp/%s_zebra.pid > ./log/%s_zebra-stdout.log 2>&1" % (r, r, r), shell=True)
        self.waitOutput()
        self.cmd("/usr/lib/frr/ripd -f ./configs/%s.conf -d -i /tmp/%s_ripd.pid > ./log/%s_ripd-stdout.log 2>&1" % (r, r, r), shell=True)
        self.waitOutput()
        self.cmd("/usr/lib/frr/bgpd -f ./configs/%s.conf -d -i /tmp/%s_bgpd.pid > ./log/%s_bgpd-stdout.log 2>&1" % (r, r, r), shell=True)
        self.waitOutput()
